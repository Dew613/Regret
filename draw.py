...
def scanline_convert(polygons, i, screen, zbuffer, color):
    ...
    x0 = minx
    temp_x0 = ((1.0*(maxx-minx))/(maxy-miny))
    z0 = minz
    temp_z0 = ((1.0*(maxz-minz))/(1.0*(maxy-miny)))
   
    if miny != midy:
        temp_x1 = ((1.0*(midx-minx))/(1.0*(midy-miny)))
        x1 = minx
        temp_z1 = ((1.0*(midz-minz))/(1.0*(midy-miny)))
        z1 = minz
    else:
        temp_x1 = ((1.0*(maxx-minx))/(1.0*(maxy-miny)))
        x1 = midx
        temp_z1 = ((1.0*(maxz-minz))/(1.0*(maxy-miny)))
        z1 = midz
    ...
    while y < maxy:
        ...
        if (y < midy and midy-y < 1):
            x0 = minx + (midy-miny)*temp_x0    #x0 += temp_x0
            x1 = midx
            y = midy
            z0 = minz + (midy-miny)*temp_z0
            z1 = midz
            draw_line(int(x0),int(y),int(z0),int(x1),int(y),int(z1),screen,zbuffer,color)
        ...
        if y == midy:
            ...
            z1 = midz
            if maxy != midy:
                temp_x1 = ((1.0*(maxx-midx))/(1.0*(maxy-midy)))
                temp_z1 = ((1.0*(maxz-midz))/(1.0*(maxy-midy)))
            else:
                temp_x1 = ((1.0*(maxx-minx))/(1.0*(maxy-miny)))
                temp_z1 = ((1.0*(maxz-minz))/(1.0*(maxy-miny)))
